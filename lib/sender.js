...
FluentSender.prototype._doConnect = function(callback) {
  ...
  if (this.path) {
    ...
    if (this.tls) {
      var tlssocket = tls.connect({ socket: this._socket, host: this.host, port: this.port });
    }
    this._socket.connect(this.port, this.host, () => {
      if (this.tls) {
        this._baseSocket = this._socket;
...
FluentSender.prototype._disconnect = function() {
  ...
  this._baseSocket && this._baseSocket.destroy();
  this._socket = null;
  this._baseSocket = null;
  this._status = null;
  this._connecting = false;
